...
namespace Plugin {
    ...
    public:
        ...
        static void FillNetworkInfo(const WPASupplicant::Network& info, JsonData::WifiControl::NetworkInfo& net)
        {
            ...
            while (pairs != 0) {
                if ((bit & pairs) != 0) {
                    ...
                    JsonData::WifiControl::PairingInfo pairing;
                    WifiControl::FillPairing(method, info.Key(method), pairing);
                    net.Pairing.Add(pairing);
                    pairs &= ~bit;
                }
                bit = (bit << 1);
            }
        ...
        static void FillPairing(const enum WPASupplicant::Network::pair method, const uint8_t keys, JsonData::WifiControl::PairingInfo& pairing)
        {
        ...
        public:
            struct Network {
                using Pairing = JsonData::WifiControl::PairingInfo;
            };
        ...
        public:
            using Config = JsonData::WifiControl::ConfigInfo;
