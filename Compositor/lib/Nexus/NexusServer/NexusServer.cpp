        ...
        if (_instance == nullptr) {
            ...
            if (config.HDCPLevel.Value() != Config::HDCP_NONE) {
                ...
                if ((config.HDCP1xBinFile.IsSet() == true) && (config.HDCP1xBinFile.Value().empty() == false)) {
                    memset(_serverSettings.hdcp.hdcp1xBinFile, 0, 128);
                    snprintf(_serverSettings.hdcp.hdcp1xBinFile, config.HDCP1xBinFile.Value().length(), "%s",  config.HDCP1xBinFile.Value().c_str());
                }

                /* set the hdcp2x bin file path */
                if ((config.HDCP2xBinFile.IsSet() == true) && (config.HDCP2xBinFile.Value().empty() == false)) {
                    memset(_serverSettings.hdcp.hdcp2xBinFile, 0, 128);
                    snprintf(_serverSettings.hdcp.hdcp2xBinFile, config.HDCP2xBinFile.Value().length(), "%s",  config.HDCP2xBinFile.Value().c_str());
                }
            }
